function Kg = constructGlobalStiffness(elements,numEqn)
			...
			for k = 1:numNodesOfElement
				for l = 1:numNodesOfElement
					if ((element.eqnNumbering(k, i) ~= 0) &&  (element.eqnNumbering(l, j) ~= 0))
						Kg(element.eqnNumbering(k, i), element.eqnNumbering(l, j)) = Kg(element.eqnNumbering(k, i), element.eqnNumbering(l, j))+ element.K(dofNumbering(k, i), dofNumbering(l, j));
					end
				end
			end
		end
